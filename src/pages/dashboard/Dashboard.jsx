  ...
  const Dashboard = () => {
    ...
    const fetchAllDashboardData = async (monthMoment = dayjs()) => {
      try {
        ...
        setTotalLeads(accountsInSelectedMonth.filter(account => !account.isCustomer).length);
        setTotalCustomers(accountsInSelectedMonth.filter(account => account.isCustomer && dayjs(account.convertedDate).isBetween(monthStart, monthEnd, null, '[]')).length);
        setTotalClosedAccounts(accountsInSelectedMonth.filter(account => account.status === 'Closed' && dayjs(account.closedDate).isBetween(monthStart, monthEnd, null, '[]')).length);

        // New Pie Chart Data Calculations (from accounts created/active in the selected month)
        const monthlyActiveLeads = accountsInSelectedMonth.filter(account => !account.isCustomer && account.status === 'Active').length;
        const monthlyWaitingLeads = accountsInSelectedMonth.filter(account => !account.isCustomer && account.status === 'Waiting').length;
        const monthlyConvertedCustomers = accountsInSelectedMonth.filter(account => account.isCustomer && dayjs(account.convertedDate).isBetween(monthStart, monthEnd, null, '[]')).length;
        const monthlyClosedAccounts = accountsInSelectedMonth.filter(account => account.status === 'Closed' && dayjs(account.closedDate).isBetween(monthStart, monthEnd, null, '[]')).length;
    ...
    const fetchAllFollowUps = async (quotations, accounts, invoices, monthMoment) => {
      ...
      const filterFollowUpsByMonth = (followUps) => {
        return followUps.filter(f => {
          ...
          return followupDate.isValid() && followupDate.month() + 1 === month && followupDate.year() === year;
        });
      };
          ...
          <Col xs={24} lg={12}>
            ...
            >
              ...
              <div className="invoice-summary-info">
                <p>
                  <strong>Total Accounts (This Month):</strong> {activeLeadsPie + waitingLeadsPie + customersPie + closedAccountsPie}
                </p>
              </div>
            </Card>
          </Col>
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 24 }}>
            ...
            <Card
              title={
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span>Monthly Leads Trend</span>
                  {/* Optional: Add a DatePicker here if you want to control this chart's month independently }
                </div>
              }
              bordered
            >
              <div className="chart-wrapper" style={{ height: 250 }}>
                <Line
                  options={{
                    ...
                    scales: {
                      x: { type: 'time', time: { unit: 'month', tooltipFormat: 'MMM.YYYY' }, title: { display: true, text: 'Month' } },
                      y: { beginAtZero: true, title: { display: true, text: 'Number of Leads' } },
                    },
                  }}
                  data={{
                    labels: leadsMonthlyTrend.map(d => d.x),
                    datasets: [{
                      label: 'New Leads',
                      data: leadsMonthlyTrend.map(d => d.y),
                      borderColor: '#4CAF50',
          ...
          <Col xs={24} lg={12}>
            <Card
              title={
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span>Monthly Customer Conversion Trend</span>
                </div>
              }
              bordered
            >
              <div className="chart-wrapper" style={{ height: 250 }}>
                <Line
                  ...
                  data={{
                    labels: customersMonthlyTrend.map(d => d.x),
                    datasets: [{
                      label: 'Converted Customers',
                      data: customersMonthlyTrend.map(d => d.y),
                      borderColor: '#2196F3',
        ...
        <Row gutter={[16, 16]} style={{ marginTop: 24 }}>
          <Col xs={24} lg={12}>
            <Card
              title={
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span>Monthly Closed Accounts Trend</span>
                </div>
              }
              bordered
            >
              <div className="chart-wrapper" style={{ height: 250 }}>
                <Line
                  options={{
                    ...
                    scales: {
                      x: { type: 'time', time: { unit: 'month', tooltipFormat: 'MMM.YYYY' }, title: { display: true, text: 'Month' } },
                      y: { beginAtZero: true, title: { display: true, text: 'Number of Closed Accounts' } },
                    },
                  }}
                  data={{
                    labels: closedAccountsMonthlyTrend.map(d => d.x),
                    datasets: [{
                      label: 'Closed Accounts',
                      data: closedAccountsMonthlyTrend.map(d => d.y),
                      borderColor: '#673AB7',
          ...
          <Form form={editForm} layout="vertical">
            ...
            >
              <Select>
                ...
                {/* Add other relevant statuses if applicable for follow-ups */}
              </Select>
            </Form.Item>
          </Form>
        </Modal>
