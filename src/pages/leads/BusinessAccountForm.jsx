...
const BusinessAccountForm = ({ visible, onClose, onSave, initialValues, allUsers, loadingUsers }) => {
    ...
    const sourceType = Form.useWatch('sourceType', form); // Keep if needed for conditional rendering
    ...
    const handleSubmit = async () => {
        try {
            ...
            delete dataToSave.noteInput; // Remove temporary form field

            // Await the onSave function, which should make the API call
            await onSave(dataToSave);

            toast.success(`Account ${initialValues ? 'updated' : 'created'} successfully!`);
            onClose(); // Close the drawer only on success

        } catch (errorInfo) {
            console.error('Validation Failed or API Error:', errorInfo);

            // Check if the error is from the API due to a duplicate business name (409 Conflict)
            if (errorInfo && errorInfo.response && errorInfo.response.status === 409 && errorInfo.response.data && errorInfo.response.data.message) {
                ...
                // This is a validation error from Ant Design's form.validateFields()
                toast.error('Please fill in all required fields correctly.');
            } else {
                // Generic error for other API issues or unexpected errors
