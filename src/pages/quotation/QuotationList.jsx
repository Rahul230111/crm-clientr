...
import { Table, Button, Tooltip, Tag, Input, Space, Popconfirm, Typography, Modal, Descriptions, Divider } from 'antd';
import { EditOutlined, DeleteOutlined, EyeOutlined, PrinterOutlined, SearchOutlined, MessageOutlined } from '@ant-design/icons';
...
const QuotationList = ({ quotations, onAddNew, onEdit, onDelete, onSearch, onViewNotes }) => {
  ...
  const generatePDF = async (record, mode = 'download') => {
    try {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Tooltip title="Download PDF">
            <Button icon={<PrinterOutlined />} onClick={() => generatePDF(record, 'download')} />
          </Tooltip>
          <Tooltip title="Notes">
  ...
  return (
    <>
      ...
      <Modal
        ...
        footer={[
          <Button key="view" icon={<EyeOutlined />} onClick={() => generatePDF(selectedQuotation, 'view')}>View PDF</Button>,
              ...
              <>
                ...
                <Table
                  ...
                  columns={[
                    ...
                    { title: 'Rate (₹)', dataIndex: 'rate', width: 120, align: 'right', render: rate => `₹${(rate || 0).toFixed(2)}` },
                    { title: 'Amount (₹)', width: 120, align: 'right', render: (_, item) => <Text strong>₹{(item.quantity * item.rate).toFixed(2)}</Text> }
                  ]}
                  pagination={false}
                  size="small"
                  rowKey={(item, idx) => idx}
