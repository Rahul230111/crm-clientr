...
import {
  Table, Button, Tooltip, Tag, Input, Space, Popconfirm,
  Typography, Modal, Descriptions, Divider
} from 'antd';
import {
  EditOutlined, DeleteOutlined, EyeOutlined,
  PrinterOutlined, SearchOutlined, MessageOutlined
...
const QuotationList = ({ quotations, onAddNew, onEdit, onDelete, onSearch, onViewNotes }) => {
    ...
    },
    {
      ...
      render: (notes) => {
        if (notes && notes.length > 0) {
          ...
          const snippet = latestNote.text.length > 30 ? `${latestNote.text.substring(0, 30)}...` : latestNote.text;
          return (
        ...
        return <Text type="secondary" italic>No notes</Text>;
      }
    },
    {
      title: 'Total (₹)',
      ...
      render: (_, record) => (
        <Space>
          ...
          <Tooltip title="Download PDF">
            <Button icon={<PrinterOutlined />} onClick={() => generateAndDownloadPDF(record)} />
          </Tooltip>
          <Tooltip title="Notes">
  ...
  return (
    <>
      ...
      <Modal
        ...
        footer={[
          <Button key="download" icon={<PrinterOutlined />} onClick={() => generateAndDownloadPDF(selectedQuotation)}>Download PDF</Button>,
        ...
        {selectedQuotation && (
          <div id={`quotation-${selectedQuotation._id}`}>
            <Descriptions column={2} bordered size="small">
              ...
              <Descriptions.Item label="Business Name">{selectedQuotation.businessName}</Descriptions.Item>
              <Descriptions.Item label="Customer Name">{selectedQuotation.customerName}</Descriptions.Item>
              ...
              <Descriptions.Item label="Business Info" span={2}>
                <Text style={{ whiteSpace: 'pre-wrap' }}>{selectedQuotation.businessInfo || 'N/A'}</Text>
              </Descriptions.Item>
              <Descriptions.Item label="Customer Address" span={2}>
                <Text style={{ whiteSpace: 'pre-wrap' }}>{selectedQuotation.customerAddress || 'N/A'}</Text>
              </Descriptions.Item>
              ...
              <>
                ...
                <Table
                  ...
                  columns={[
                    ...
                    { title: 'HSN/SAC', dataIndex: 'hsnSac', width: 100, align: 'center' },
                    ...
                    },
                    {
                      ...
                      render: (_, item) => <Text strong>₹{(item.quantity * item.rate).toFixed(2)}</Text>
                    }
                  ]}
                  pagination={false}
                  size="small"
      ...
      {quotations.map(quotation => (
        ...
        >
          <Descriptions column={2} bordered size="small">
            ...
            <Descriptions.Item label="Business Name">{quotation.businessName}</Descriptions.Item>
            <Descriptions.Item label="Customer Name">{quotation.customerName}</Descriptions.Item>
            <Descriptions.Item label="GSTIN" span={2}>
              <Text code>{quotation.gstin || 'N/A'}</Text>
            </Descriptions.Item>
            <Descriptions.Item label="Business Info" span={2}>
              <Text style={{ whiteSpace: 'pre-wrap' }}>{quotation.businessInfo || 'N/A'}</Text>
            </Descriptions.Item>
            <Descriptions.Item label="Customer Address" span={2}>
              <Text style={{ whiteSpace: 'pre-wrap' }}>{quotation.customerAddress || 'N/A'}</Text>
            </Descriptions.Item>
            <Descriptions.Item label="Total Amount" span={2}>
              <Text style={{ fontSize: '18px', fontWeight: 'bold', color: '#52c41a' }}>
